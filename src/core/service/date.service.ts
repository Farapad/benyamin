...
export class DateService {
    ...
    addDay(date: string, day) {
        ...
        let text = '';
        try {
            let startdate = moment(date).add({day:day,hour:3,minute:30});
            text = startdate.format( 'jYYYY/jMM/jDD');
        } catch (error) {
            text = 'تاریخ نامعتبر';
        }
        return text;
    ...
    dayOfweek(time) {
        let date = moment(time).add({hour:3,minute:30});;
        let weekday = [
    ...
    hoursTodays(hour) {
        ...
        const result = Day + ' ' + i18n.global.tc('day') + ' ' + Hours + ' ' + i18n.global.tc('hour');
        return result
    }
    ...
    calculateNumberOfHoursJs(date) { //محاسبه تعداد ساعت رد شده
        ...
        const seconds = Math.floor((Number(dateFuture) - Number((dateNow))) / 1000);
    ...
    addDifUtc(date){
        let dt = moment(date);
        if (this.getCurrentJalaliMonth() > 6) dt = dt.add(3.5, 'hours');
        else dt = dt.add(4.5, 'hours');
        return dt;
    }
