...
export class DataFormatterService {
    ...
    formatCurrency(price ,decimalDigits = false) {
        ...
        if (Number(price) > 0) {
            if (decimalDigits) {
                ...
                var re = '\\d(?=(\\d{' + (3) + '})+' + (4 > 0 ? '\\.' : '$') + ')';
                const finalNumber = n.toFixed(Math.max(0, ~~(y ?? 1))).replace(new RegExp(re, 'g'), '$&,');
                
                return Number(finalNumber) % 1 !== 0 ? finalNumber : Math.round(Number(finalNumber));
    ...
    passedTime(date) {
        return moment(date).add({hour:3,minute:30}).fromNow(true);
    }

    showCount(number) {
        if (number < 1000) {
          return {value:number,key:null};
        }
        if (number < 1000000) {
          ...
          return {value:number,key:'K'};;
        }
        const mod = number % 1000000;
        if (mod >= 500) {
          number = Math.floor(number / 1000000);
        } else {
          number = Math.ceil(number / 1000000);
        }
        return {value:number,key:'M'};;
      }
